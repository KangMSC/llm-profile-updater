...
## 2025년 10월 9일 (저녁) - 이벤트 조회 로직 개선

### 기능 개선

1.  **나레이션 이벤트 항상 포함**:
    -   **목표**: 프로필 업데이트 및 일기 생성 시, 캐릭터의 행동뿐만 아니라 주변 상황을 설명하는 나레이션(`event_type = 'direct_narration')을 항상 참고하여 더 풍부한 컨텍스트를 LLM에 제공하도록 합니다.
    -   **구현 내용**:
        1.  **`db.js` 수정**: `getCharacterEvents` (프로필 업데이트용) 및 `getEventsForDiary` (일기 생성용) 함수 내의 SQL 쿼리를 수정했습니다.
        2.  `WHERE` 절에 `OR event_type = 'direct_narration'` 조건을 추가하여, 기존에 조회하던 캐릭터 관련 이벤트에 더해 모든 나레이션 이벤트를 함께 가져오도록 변경했습니다. (일기 생성을 위한 쿼리에서는 해당 날짜의 나레이션만 포함되도록 날짜 조건을 유지했습니다.)