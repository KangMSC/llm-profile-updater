...
## 2025년 10월 11일 (오후) - 미디어(동영상) 지원 및 UI 개선

### 기능 추가 및 개선

1.  **동영상 및 GIF 업로드 지원**:
    *   **목표**: 기존 이미지 업로드 기능을 확장하여 짧은 MP4 동영상과 움직이는 GIF 파일을 지원합니다.
    *   **구현 내용**:
        1.  **`multer` 설정 업데이트**: `server.js`에서 `multer` 설정을 수정하여, 파일의 MIME 타입에 따라 이미지는 `public/images`, 동영상은 `public/videos` 디렉터리에 분리하여 저장하도록 변경했습니다. 또한, 허용 파일 목록에 `video/mp4` 등을 추가하고 파일 크기 제한을 5MB로 설정했습니다.
        2.  **통합 미디어 렌더링**: 캐릭터 페이지 로딩 시, 이미지와 비디오 디렉토리를 모두 스캔하여 타임스탬프 기준으로 정렬한 후, 가장 최신 미디어를 프로필의 대표 미디어로 표시하도록 로직을 통합했습니다. 최신 미디어가 MP4일 경우, 자동 재생되는 `<video>` 태그로, 그 외에는 `<img>` 태그로 조건부 렌더링됩니다.
        3.  **클라이언트 로직 수정**: `public/js/character-page.js`의 갤러리 및 모달 로직을 리팩토링하여, 이미지와 비디오를 모두 원활하게 처리하도록 수정했습니다. 모달 창을 닫으면 비디오 재생이 자동으로 멈춥니다.

2.  **미디어 표시 UI 개선**:
    *   **문제 1**: 히스토리 갤러리에서 동영상이 'MP4'라는 텍스트로만 표시되어 내용을 알 수 없었습니다.
    *   **해결 1**: 히스토리 갤러리에서 텍스트 대신, 음소거된 작은 `<video>` 태그를 직접 렌더링하여 동영상의 첫 프레임이 보이도록 개선했습니다.
    *   **문제 2**: 히스토리 썸네일이 잘리고, 원본 보기 모달에서는 미디어가 화면을 넘어갈 정도로 크게 표시되었습니다.
    *   **해결 2**: 썸네일과 모달 양쪽의 CSS에서 `object-fit` 속성을 `cover`에서 `contain`으로 변경하고, 모달 컨텐츠의 최대 크기를 뷰포트 기준으로(90vw, 90vh) 명확히 제한하여, 모든 미디어가 잘리거나 넘치지 않고 비율에 맞게 표시되도록 수정했습니다.

---

## 2025년 10월 11일 (저녁) - .txt 프로필 생성 연동

### 기능 개선

1.  **최초 프로필 생성 시 `.txt` 파일 동시 생성**:
    *   **목표**: '최초 프로필 생성' 기능 실행 시, `.json` 프로필뿐만 아니라 상위 프로그램 호환을 위한 `.txt` 프로필도 함께 생성되도록 기능을 보완했습니다.
    *   **구현 내용**: `server.js`의 프로필 생성 API 로직을 수정하여, `fs.writeFile`로 `.json` 파일을 저장한 직후, `src/main.js`로부터 불러온 `formatProfileToTxt` 함수를 호출하여 `generated-profiles` 디렉터리에 `.txt` 파일을 저장하도록 했습니다.
