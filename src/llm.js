...
async function updateCharacterProfile(characterName, events, existingProfileJson, instructions, customKeys = []) {
  ...
  try {
    await logger.clearLog(characterName, 'error', action);
    ...
    await logger.writeLog(characterName, 'prompt', action, fullPrompt);
    ...
    await logger.writeLog(characterName, 'error', action, errorMessage);
    console.error(`Error during ${action} for ${characterName}:`, errorMessage);
    throw error;
  }
}

async function generateCharacterDiary(characterName, events, profileJson, customKeys = []) {
  const action = 'generate_diary';
  try {
    await logger.clearLog(characterName, 'error', action);
...
async function generateSdPrompt(characterName, profileJson) {
  const action = 'generate_sd_prompt';
  try {
    await logger.clearLog(characterName, 'error', action);
...
async function generateInitialProfile(characterName, userPrompt, allKeys, customKeys = []) {
  const action = 'generate_initial_profile';
  let content; // Declare content here to make it accessible in the catch block
  try {
    await logger.clearLog(characterName, 'error', action);
    ...
    const finalUserPrompt = substitutePrompt(userPromptTemplate, {
        characterName,
        userPrompt,
    ...
    if (error.response) {
        ...
        errorMessage = `SyntaxError: Failed to parse LLM response as JSON. Raw response:\n\n${content}`;
    }
    await logger.writeLog(characterName, 'error', action, errorMessage);
    console.error(`Error during ${action} for ${characterName}:`, errorMessage);
    throw error;
